package com.mundo.netty.demo.client;

import com.mundo.netty.demo.client.handler.NettyClientHandlerInitializer;
...
public class NettyClient {
     ...
     * @param message 消息体
     */
    public void send(MessageBase.Message message) {
        if (channel == null) {
            logger.error("[send][连接不存在, 正在重新连接服务端]");
            reconnect();
            return;
        }
        if (!channel.isActive()) {
            logger.error("[send][连接({})未激活, 正在重新连接服务端]", channel.id());
            reconnect();
            return;
        }
        // 发送消息
