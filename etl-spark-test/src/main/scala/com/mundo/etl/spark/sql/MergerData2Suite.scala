...
object MergerData2Suite {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val seq = List(
      ...
      ("b", "test5555", "test6666", "2017-12-05 12:12:45"))

    val dataFrame = seq.toDF("colmun1", "colmun2", "colmun3", "createTime")
    ...
    mappedRdd.reduceByKey((item1, item2) => {
      ...
      for (i <- 0 until maxd) {
        ...
        if (!itemValue1.equalsIgnoreCase(itemValue2)) {
          if (itemValue1 == "" || itemValue2 == "") {
            result += itemValue1 + itemValue2 + "MM"
          } else {
            if (date1.getTime > date2.getTime) {
              result += itemValue1 + "MM"
            } else {
              result += itemValue2 + "MM"
            }
          }
        } else {
          result += itemValue1 + "MM"
      ...
      val splitsed = result.split("MM")
      val row = RowFactory.create(splitsed(0), splitsed(1), splitsed(2), splitsed(3))
      row
    }).map(_._2).map(item => {
