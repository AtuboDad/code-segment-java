...
/**
  ...
  * Date: 2017-12-11
  */
object MergerDataIntoHiveTable3 {

  private val  logger = LoggerFactory.getLogger("MergerDataIntoHiveTable3")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mappedRdd = dataFrame.rdd.map(item => {
      ...
      val bizdateStr = item.getString(bizdateIndex)
      val retaileridStr = item.getString(retaileridIndex)
      ...
      if (key.contains(",")) {
        ...
        (keyIndexItemResult, item.mkString(connectStr) + connectStr + bizdateStr + connectStr + retaileridStr)
      } else {
        (splitsed(keyIndex), item.mkString(connectStr) + connectStr + bizdateStr + connectStr + retaileridStr)
      }
    })
...
//    logger.info(mappedRdd.collect().mkString(","))
    ...
    val keyStringRDD = mappedRdd.reduceByKey((item1, item2) => {
      ...
      // 获取最后一个值索引值(站点ID字段索引)
      val item1Retailerid = itemSplit1.last
      val item2Retailerid = itemSplit2.last
      if (item1Retailerid.trim == "" || item2Retailerid.trim == "") {
        if (item1Retailerid.trim == "") {
          logger.info("item1Retailerid  字段值为空")
        }
        if (item2Retailerid.trim == "") {
          logger.info("item2Retailerid  字段值为空")
        }
      }

      // 获取时间字段值
      var item1Bizdate = itemSplit1(itemSplit1.length - 2)
      var item2Bizdate = itemSplit2(itemSplit2.length - 2)
