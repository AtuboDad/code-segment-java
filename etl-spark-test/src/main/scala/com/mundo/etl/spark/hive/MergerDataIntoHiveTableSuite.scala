...
/**
  ...
  * Date: 2017-12-11
  */
object MergerDataIntoHiveTableSuite {

  private val logger = LoggerFactory.getLogger("MergerDataIntoHiveTableSuite")

  val connectStr = "MM&"
  val connectStr2 = "M@M"

  private def getFromCollect(policyCollect: Array[String], itemRetailerid: String, index: Int) = {
    var collectItemStr = ""
    // 通过策略表信息，过去站点对应的优先级以及过期时间天数
    if (policyCollect.mkString(",").contains(itemRetailerid)) {
      for (collectItem <- policyCollect) {
        if (collectItem.contains(itemRetailerid) || collectItem.startsWith(itemRetailerid)) {
          collectItemStr = collectItem
        }
      }
    }
    collectItemStr.split(connectStr2)(index).toInt

//    var collectItemResult = 90
//    for (collectItem <- policyCollect) {
//      if (collectItem.contains(itemRetailerid)) {
//        collectItemResult = collectItem.split(connectStr2)(index).toInt
//      }
//    }
//
//    collectItemResult
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 1) {
      ...
      println("       --class com.mundo.etl.spark.hive.MergerDataIntoHiveTableSuite \\")
    ...
    val conf = new SparkConf().setAppName("MergerDataIntoHiveTableSuite")
    // 初始化SparkContext
    ...
    val policyCollect = policyDF.select("retailerid", "priority", "validity").map(row => {
      ...
      val result = retailerid + connectStr2 + priority + connectStr2 + validity + connectStr2
      result
    }).collect()
    ...
    val keyStringRDD = mappedRdd.reduceByKey((item1, item2) => {
      ...
      val priority1 = getFromCollect(policyCollect, item1Retailerid, 1)
      val validity1 = getFromCollect(policyCollect, item1Retailerid, 2)

      // 获取优先级 获取过期时间天数
      val priority2 = getFromCollect(policyCollect, item2Retailerid, 1)
      val validity2 = getFromCollect(policyCollect, item2Retailerid, 2)
