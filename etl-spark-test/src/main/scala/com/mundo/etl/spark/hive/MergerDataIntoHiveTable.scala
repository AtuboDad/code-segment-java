...
import com.mundo.etl.spark.utils.{CreateRowUtilV2, CreateRowUtilV3}
import com.mundo.etl.spark.utils.{CreateRowUtilV2, CreateRowUtilV3}
...
/**
  ...
  * Date: 2017-12-11
  */
object MergerDataIntoHiveTable {

  private val logger = LoggerFactory.getLogger("MergerDataIntoHiveTable")
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 1) {
      ...
      println("       <dir>/etl-spark-1.0-SNAPSHOT.jar [table_name] [partition]")
      println("")
      println("       like: ")
      println("            table_name: edw.f_trademark_info")
      println("            partition: 200")
      System.exit(0)
    }
...
//    val rows = policyDF.collect()
//    if (rows.length <= 0) {
//      println("Error message: It's not exist this primary key.")
//      System.exit(0)
//    }
    // 策略表中的Primary key
    val key = policyDF.first().getString(1)
    ...
    var dataFrameRdd = dataFrame.rdd
    if (partition > 0) {
      dataFrameRdd = dataFrameRdd.repartition(partition)
    }
    val mappedRdd = dataFrameRdd.map(item => {
      // 获取时间字段索引
    ...
    val keyStringRDD = mappedRdd.reduceByKey((item1, item2) => {
      ...
      // 转化成时间格式
      if (date1Str.trim == "" || date1Str.equalsIgnoreCase(nullString) || !isValidDate(date1Str)) {
        date1Str = "20161212"
      }
      if (date2Str.trim == "" || date2Str.equalsIgnoreCase(nullString) || !isValidDate(date2Str)) {
        date2Str = "20161212"
      }
      ...
      for (collectItem <- policyCollect) {
        if (collectItem.contains(item1Retailerid.trim)) {
          priority1 = collectItem.split(connectStr)(1).toInt
          validity1 = collectItem.split(connectStr)(2).toInt
        }
        if (collectItem.contains(item2Retailerid.trim)) {
          priority2 = collectItem.split(connectStr)(1).toInt
          validity2 = collectItem.split(connectStr)(2).toInt
        }
      }
      ...
      for (i <- 0 until maxd) {
        ...
        if (itemValue1 == null || itemValue2 == null) {
          if (itemValue1 == null) {
            if (itemValue2 == null) {
              itemValue2 = nullString
            }
            result += itemValue2 + connectStr
          } else {
            if (itemValue1 == null) {
              itemValue1 = nullString
            }
            result += itemValue1 + connectStr
          }
        } else if ("".equalsIgnoreCase(itemValue1) || "".equalsIgnoreCase(itemValue2) || nullString.equalsIgnoreCase(itemValue1) || nullString.equalsIgnoreCase(itemValue2)) {
          if (nullString.equalsIgnoreCase(itemValue1) || nullString.equalsIgnoreCase(itemValue2)) {
            if (nullString.equalsIgnoreCase(itemValue1)) {
              result += itemValue2 + connectStr
            } else {
              result += itemValue1 + connectStr
            }
          } else {
            if ("".equalsIgnoreCase(itemValue1) && "".equalsIgnoreCase(itemValue2)) {
              result += nullString + connectStr
            } else {
      ...
      if (maxd >= 38) {
        val columnBean = CreateRowUtilV2.createRow(maxd, resultSplits)
        (columnBean, item1._2, item1._3)
      } else {
        val columnBean = CreateRowUtilV3.createRow(maxd, resultSplits)
        (columnBean, item1._2, item1._3)
      }
    ...
    // 保存至Hive表中
    var resultDF = sqlContext.createDataFrame(valueRDDs, dataFrame.schema)
    //    resultDF.write.saveAsTable(args(0) + "_sparksql")
//    resultDF.repartition(200).write.parquet("/tmp/read-hive/parquet/" + args(0) + "_sparksql")
//    val repartitionRDD = resultDF.repartition(4)
    if (partition > 0) {
      resultDF = resultDF.repartition(partition)
    }
  ...
  private def isValidDate(str: String) = {
